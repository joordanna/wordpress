...
class Webhook {
	...
	private function process_payment() {
		...
		try {
			...
			if ( null !== $this->subscription && 'active' !== $this->subscription->get_status() && false === $this->subscription->can_be_updated_to( 'active' ) ) {
				...
				throw new Event_Exception( sprintf( __( 'Prevents 500 error from WooCommerce Subscriptions: unable to change subscription status to %s.', 'woo-asaas' ), 'active' ) );
			} elseif ( null !== $this->subscription ) {
				throw new Event_Exception( $error );
			} else {
	...
	private function on_payment_deleted() {
		...
		try {
			...
			throw new Event_Exception( __( 'PAYMENT_DELETED: prevents 500 error from WooCommerce Subscriptions: unable to change subscription/order status to cancelled.', 'woo-asaas' ) );
		}
	...
	private function on_payment_overdue() {
		...
		try {
			...
			throw new Event_Exception( __( 'PAYMENT_OVERDUE: prevents 500 error from WooCommerce Subscriptions: unable to change subscription/order status to failed.', 'woo-asaas' ) );
		}
	...
	private function on_payment_refunded() {
		...
		try {
			...
			throw new Event_Exception( __( 'PAYMENT_REFUNDED: prevents 500 error from WooCommerce Subscriptions: unable to change subscription/order status to refunded.', 'woo-asaas' ) );
		}
	...
	private function on_payment_restored() {
		...
		try {
			...
			throw new Event_Exception( __( 'PAYMENT_RESTORED: prevents 500 error from WooCommerce Subscriptions: unable to change subscription/order status to pending.', 'woo-asaas' ) );
		}
